    ...
    findDoc(db, collection, appVersion, function(results) {
      if (results.length === 0) {
        insertDoc(db, collection, appVersion, function() {
          ...
          exec(makenewCOMMAND, function(error, stdout, stderr) {
            if (stderr) {
              ...
              return logger.error("Core:accessDB::" + stderr);
            }
          ...
          exec(moveCOMMAND, function(error, stdout, stderr) {
            if (stderr) {
              ...
              return logger.error("accessDB::" + stderr);
            }
        ...
        try {
          ...
          if (stderr) { logger.error("accessDB::" + stderr); }
          logger.debug("accessDB::Deleted JSON5: " + tempCommentPath);
        }
        catch (ex) {
