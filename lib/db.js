...
var accessDB = function(appVersion, outputDir, dest, htmlPath, dirPATH, tempCommentPath, dirpluginPATH, callback) {
            ...
            movesem.take(function() {
              ...
              exec(moveCOMMAND, function(error, stdout, stderr) {
                ...
                if (fs.existsSync(htmlPath)) {
                  moveHTMLCOMMAND = "mv " + htmlPath + ' ' + dirpluginPATH + appVersion.name + '-' + appVersion.verc + '.html';
                  exec(moveHTMLCOMMAND, function(error, stdout, stderr) {
                    ...
                    if (tempCommentPath != null) {
                      moveCommentCOMMAND = "mv " + tempCommentPath + ' ' + dirpluginPATH + appVersion.name + '-' + appVersion.verc + '.json5';
                      exec(moveCommentCOMMAND, function(error, stdout, stderr) {
                        if (stderr || error) {
                          movesem.leave();
                          return logger.error("accessDB::stderr:" + stderr + ". Error:" + error);
                        }
                        runScraper(dirPATH, appVersion, function() {
                          movesem.leave();
...
var runScraper = function(path, appVersion, callback) {
  var scraper = config.get('scraper'),
      COMMAND = scraper + " " + appVersion.name + '-' + appVersion.verc + " -o " + path;
  logger.info("accessDB::runScraper:Scraping data for " + appVersion.name + " saved into " + path);
  exec(COMMAND, function(error, stdout, stderr) {
    if (stderr || error) {
      movesem.leave();
      return logger.error("accessDB::runScraper:" + stderr + ". Error:" + error);
    }
    logger.info("accessDB::runScraper:Finished scraping data for " + appVersion.name + " saved into " + path);
    callback();
  });
}
