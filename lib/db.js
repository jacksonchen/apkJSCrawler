...
var accessDB = function(appVersion, outputDir, dest, htmlPath, newhtmlPATH, dbArray) {
  var hostName = dbArray[0],
      portNumber = dbArray[1],
      dbName = dbArray[2],
      collectionName = dbArray[3];
  ...
  MongoClient.connect(mongoURL, function(err, db) {
    ...
    findDoc(db, collection, appVersion, function(results) {
      if (results.length === 0) {
        ...
        DELETEHTML = "rm " + __dirname + "/" + htmlPath;
        exec(DELETEHTML, function(error, stdout, stderr) {
          if (stderr) return console.log(stderr);
          COMMAND = "rm " + __dirname + "/" + dest;
