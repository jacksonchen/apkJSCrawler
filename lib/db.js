    ...
    findDoc(db, collection, appVersion, function(results) {
      if (results.length === 0) {
        makeDir(appVersion, function() {
          insertDoc(db, collection, appVersion, function(err) {
            if (err) {
...
var insertDoc = function(db, collection, appVersion, callback) {
  if (fs.existsSync(appVersion.path)) {
    collection.insert({'n': appVersion.name, 'verc': appVersion.verc, 'vern': appVersion.vern, 'path': appVersion.apkpath, 'd': appVersion.date, 'id': appVersion.ID, 'size': appVersion.size }, function(err, r) {
      if (err) {
        callback(true);
      }
      else {
...
var makeDir = function(appVersion, callback) {
  makenewCOMMAND = "mkdir -p " + appVersion.vercpath;
  exec(makenewCOMMAND, function(error, stdout, stderr) {
    if (stderr || error) {
      logger.error("accessDB::Making new directory failed for " + appVersion.vercpath);
      return logger.error("accessDB::stderr:" + stderr + ". Error:" + error);
    }
    logger.info("accessDB::Successfully made directory " + appVersion.vercpath);
    callback();
  });
}

var moveAPK = function(dest, appVersion, movesem, callback) {
  moveCOMMAND = "mv " + dest + ' ' + appVersion.apkpath;
  exec(moveCOMMAND, function(error, stdout, stderr) {
    if (stderr || error) { logger.error("accessDB::" + stderr); movesem.leave(); }
    logger.info("accessDB::Moved APK to " + appVersion.apkpath);
    callback();
  });
}
