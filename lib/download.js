...
var parseBody = function(html, outputDir, settings, db, title) {
  $ = cheerio.load(html);
  ...
  links.forEach(function(linkBlock) {
    downloadAPK(linkBlock, title, counter, outputDir, function(dest) {
      packageReader(outputDir, settings, dest, function(appVersion) {
        dbRead(appVersion, settings.collection, outputDir, dest, db);
      });
    })
    counter += 1;
  });
...
var download = function(url, outputDir, settings, db) {
  getHTML(url, outputDir, function(html, title) {
    parseBody(html, outputDir, settings, db, title);
  });
}
