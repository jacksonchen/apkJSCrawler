...
var updateHTML = function(appVersion) {
...
var parseBody = function(html, outputDir, settings, title, htmlPath) {
  ...
  links.forEach(function(linkBlock) {
    downloadAPK(linkBlock, title, counter, outputDir, function(dest) {
      packageReader(outputDir, settings, dest, function(appVersion, newhtmlPATH) {
        dbRead(appVersion, settings.collection, outputDir, dest, settings, htmlPath, newhtmlPATH);
      });
    })
    counter += 1;
  });
