...
function readKeyword(keyword, output_dir, pluginPath, action, callback) {
  ...
  plugin.init(keyword, output_dir, function(tempAPKdest, htmlPath, tempCommentPath, last) {
    if (tempCommentPath != null && htmlPath != null && tempCommentPath != null) {
      crawler.packageReader(output_dir, tempAPKdest, htmlPath, function(appVersion, dirPATH, dirpluginPATH) {
        crawler.dbRead(appVersion, output_dir, tempAPKdest, htmlPath, dirPATH, tempCommentPath, dirpluginPATH, function() {
          if (action === "file") {
            callback(last);
          }
        });
