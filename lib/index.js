...
function readKeyword(keyword, output_dir, pluginPath, action, callback) {
  ...
  plugin.init(keyword, output_dir, appsem, function(err, tempAPKdest, htmlPath, tempCommentPath, last) {
    if (tempCommentPath != null && htmlPath != null && tempCommentPath != null) {
      crawler.packageReader(output_dir, tempAPKdest, htmlPath, function(err, appVersion, dirpluginPATH) {
        if (err) {
          appsem.leave();
          if (action === "file") {
            callback(last);
